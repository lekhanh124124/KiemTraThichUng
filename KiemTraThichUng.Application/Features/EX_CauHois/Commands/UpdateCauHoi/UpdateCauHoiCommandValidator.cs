...
{
    public class UpdateCauHoiCommandValidator : AbstractValidator<UpdateCauHoiCommand>
    {
        private readonly INganHangCauHoiRepository _nganHangCauHoiRepository;
        public UpdateCauHoiCommandValidator(INganHangCauHoiRepository nganHangCauHoiRepository)
        {
            ...
            RuleFor(x => x)
                ...
                {
                    var Ids = new List<int>() { (int)command.Id! };
                    var (exists, missingIds) =
                        await _nganHangCauHoiRepository
                            .KiemTraTonTaiCauHoiByIdsAsync(Ids, cancellationToken);

                    if (!exists)
                    {
                        context.AddFailure("Không tìm thấy câu hỏi");
                    }
                });
