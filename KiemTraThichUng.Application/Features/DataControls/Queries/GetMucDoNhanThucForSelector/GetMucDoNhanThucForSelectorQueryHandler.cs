...
{
    public class GetMucDoNhanThucForSelectorQueryHandler 
        : IRequestHandler<GetMucDoNhanThucForSelectorQuery, ApiResponse<IReadOnlyList<SelectorItemResponse>>>
    {
        public async Task<ApiResponse<IReadOnlyList<SelectorItemResponse>>> Handle(
            GetMucDoNhanThucForSelectorQuery request, 
            CancellationToken cancellationToken)
        {
            var items = Enumeration.GetAll<MucDoNhanThuc>()
